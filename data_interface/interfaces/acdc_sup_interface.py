...
class DatasetInterface(object):
    ...
    def __init__(self, root_dir, input_size):
        ...
        self.y_train_oh = np.load(os.path.join(root_dir, 'preprocessed/{0}/sup_mask_{0}_oh.npy'.format('train')))
        ...
        self.y_validation_oh = np.load(os.path.join(root_dir, 'preprocessed/{0}/sup_mask_{0}_oh.npy'.format('validation')))
        ...
        self.y_test_oh = np.load(os.path.join(root_dir, 'preprocessed/{0}/sup_mask_{0}_oh.npy'.format('test')))
    ...
    @staticmethod
    def _data_augmentation_ops(x_train, y_train, y_train_oh):
    ...
    def get_data(self, b_size, augment=False, standardize=False, repeat=False, num_threads=4, seed=None):
        ...
        with tf.name_scope('acdc_data'):
            ...
            train_data = tf.data.Dataset.from_tensor_slices((_train_images, _train_masks, _train_masks_oh))
            valid_data = tf.data.Dataset.from_tensor_slices((_valid_images, _valid_masks, _valid_masks_oh))
            test_data = tf.data.Dataset.from_tensor_slices((_test_images, _test_masks, _test_masks_oh))
            ...
            _input_data, _output_data, _output_data_oh = iterator.get_next()
            ...
            with tf.name_scope('output_sup'):
                output_data = tf.reshape(_output_data, shape=[-1, self.input_size[0], self.input_size[1], 1])
                output_data = tf.cast(output_data, tf.float32)

            with tf.name_scope('output_sup_oh'):
                output_data_oh = tf.reshape(_output_data_oh, shape=[-1, self.input_size[0], self.input_size[1], 4])
                output_data_oh = tf.cast(output_data_oh, tf.float32)
